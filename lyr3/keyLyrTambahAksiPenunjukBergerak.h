...
void keyAksi_Penunjuk_Bergerak ()
{
  ...
  n = 0;
	  ...
	  switch( lex.type )
	    ...
	    case TYPE_IDENT:
              ...
              if(aksidat.datatype == 40) {
                ...
                Error("bentuk perintah Aksi_Tekan_Penunjuk salah");
			  }
	    ...
	    default:
	      Error("bentuk perintah Aksi_Penunjuk_Bergerak salah");
	    }
    ...
    if(n != 2) {
      Error("parameter Aksi_Penunjuk_Bergerak tidak lengkap");
    } else {
	  // printf("Aksi_Tekan_Papanketik type1 %d\n", tmpdat.GUItype);
	  // printf("Jenis Fungsi %d\n", type_func);

      // add Aksi_Tekan_Penunjuk
      gtk_widget_add_events(aksidat.widget, GDK_POINTER_MOTION_MASK);
	  ...
	  if(type_func == 1) {
        ...
        if(aksidat.datatype == 40) {
                  ...
                  g_object_set_data(G_OBJECT(aksidat.widget), "lbl_aksi_penunjuk_bergerak", dat);  // GTK3

		  if(aksidat.isGUI == 1) {        // window widget
		    g_signal_connect_swapped(G_OBJECT(aksidat.widget), "motion-notify-event",
				G_CALLBACK(Do_AksiBendaFungsiMouse), aksidat.widget);
		  }

		} else {
          Error("parameter pada kode perintah Aksi_Penunjuk_Bergerak salah");
		}
        ...
        if(aksidat.datatype == 40) {
		  ...
		  if(aksidat.isGUI == 1) {        // window widget
		    g_signal_connect_swapped(G_OBJECT(aksidat.widget), "motion-notify-event",
		    G_CALLBACK(Do_AksiFungsiMoveMouse), aksidat.widget);
		  }
