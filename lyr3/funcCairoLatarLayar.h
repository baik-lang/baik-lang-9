  ...
  memset(&class_tmpvar, '\0', sizeof(class_tmpvar));
      ...
      do {
         ...
         if(lex.type == TYPE_NUM) {
             ...
             Error("latar_layar: masukan data salah");
             ...
             if(strlen(lex.detail.array_str) > 0) {
              ...
              if(valdat.datatype == 6 ) {
                if(valdat.array_i == '\0')
                  Error("latar_layar: ukuran Untaian belum terdefinisi");
             ...
             if(valdat.datatype == 40){               /* Window Widget */
			   memset(&tmpdat, '\0', sizeof(tmpdat));
               tmpdat = valdat;
               tmpdat.datatype = 40;

			 } else
             if(valdat.datatype == 83){               /* Pixmap */
			   memset(&tmpdat, '\0', sizeof(tmpdat));
               tmpdat = valdat;
               tmpdat.datatype = 83;
      ...
      if(n!=4) {
        ...
        if(valdat.datatype == 40){
		  ...
		  //printf("funcCairo LatarLayar ...\n");

		  datx.cr_img = gdk_cairo_create(gtk_widget_get_window(valdat.widget)); // from kanvas using cairo
		  datx.datatype = 80; // cairo_t

		  //gdk_window_get_size(gtk_widget_get_window(valdat.widget), &w, &h);
		  gtk_widget_get_size_request(valdat.widget, &h, &w);


		} else if(valdat.datatype == 83) {

		  //datx.cr_img = gdk_cairo_create(valdat.pixmap); // from pixmap using cairo
		  //datx.datatype = 80; // cairo_t
		  //gdk_drawable_get_size(valdat.pixmap, &w, &h);

          printf("create cr img from cairo_surface_t\n");
		  datx.cr_img = cairo_create(valdat.cr_surface); // from cairo_surface_t using cairo
		  datx.datatype = 80; // cairo_t
		  //gdk_drawable_get_size(valdat.pixmap, &w, &h);
