    ...
    if(n != 2) {
	  ...
	  if(type_func == 1) {
        ...
        if(aksidat.datatype == 40) {
			...
			g_object_set_data(G_OBJECT(aksidat.widget), "cairo_aksi_ekspos", dat);  // GTK3

			if (aksidat.isGUI == 1) {        // window widget
				g_signal_connect_swapped(aksidat.widget, "draw",
					G_CALLBACK(Do_AksiFungsiDraw), aksidat.widget);
