...
void keyLembarBuku ()
{
  ...
  VAL_LABEL valdat,valdat2,tmpdat, tmpdat2,tmpdat3, tmpdat4;
  ...
  memset(&tmpdat4, '\0', sizeof(tmpdat4));
  ...
  // printf("keyLembarBuku\n");
	    ...
	    case TYPE_IDENT:
              ...
              if(valdat.datatype == 40) {
                if(n == 0) {
				  ...
				  tmpdat3.widget   = valdat.widget;
                  tmpdat3.datatype = 40;
				  tmpdat3.GUItype  = valdat.GUItype;
				}
			  } else {
                Error("bentuk perintah Lembar_Buku salah");
			  }
              ...
              if(strlen(lex.detail.array_str) > 0) {
                ...
                if(isdigit(valdat.array_idx[0])) {
                   ...
                   if(currentClass != NULL && strlen(currentClass) > 0) {
                     ...
                     tmpdat4 = ValLabel( class_tmpvar, class_sub_deep, tmpdat4, VAL_FLAG_SEARCH_R );
				   } else {
                     tmpdat4 = ValLabel( valdat.array_idx, sub_deep, tmpdat4, VAL_FLAG_SEARCH_R );
				     // printf("get idx type: %d, val %d\n", tmpdat.datatype, tmpdat.val);
   		           }

                   if(tmpdat4.datatype==0)
                     idx = tmpdat4.val;
                   else
		  ...
		  memset(&class_tmpvar, '\0', sizeof(class_tmpvar));
               ...
               // ltoa(idx, valdat2.array_idx, 10);
                ...
                if(valdat2.datatype == 60 && n == 0) {           // Array GUI 
                  ...
                  tmpdat2.datatype = 40;
				} else if(valdat2.datatype == 60 && n == 2) {    // Array GUI
                  memset(&tmpdat3, '\0', sizeof(tmpdat3));
                  tmpdat3.widget   = get_gui_array(valdat2, idx);
				  tmpdat3.eventbox = get_eventbox_array(valdat2, idx);
				  tmpdat3.GUItype  = get_guitype_array(valdat2, idx);
                  tmpdat3.datatype = 40;
                } else {
    ...
    if(n != 3) {
      Error("parameter Lembar_Buku tidak lengkap");
    } else {
	  gtk_notebook_append_page (GTK_NOTEBOOK (tmpdat.widget), tmpdat2.widget, tmpdat3.widget);
    }
