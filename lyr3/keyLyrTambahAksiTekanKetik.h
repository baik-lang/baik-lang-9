...
void keyAksi_Tekan_Papanketik ()
{
  ...
  n = 0;
    ...
    if(n != 2) {
      Error("parameter Aksi_Tekan_Papanketik tidak lengkap");
    } else {
	  // printf("Aksi_Tekan_Papanketik type1 %d\n", tmpdat.GUItype);
	  // printf("Jenis Fungsi %d\n", type_func);

      // add Aksi_Tekan_Papanketik
      gtk_widget_add_events(aksidat.widget, GDK_KEY_PRESS_MASK);
	  ...
	  if(type_func == 1) {
        ...
        if(aksidat.datatype == 40) {
          ...
          g_object_set_data(G_OBJECT(aksidat.widget), "lbl_aksi_presskeyboard", dat);  // GTK3

		  if(aksidat.isGUI == 1) {        // window widget
		    g_signal_connect_swapped(G_OBJECT(aksidat.widget), "key-press-event",
		    G_CALLBACK(Do_AksiBendaFungsiPressKeyboard), aksidat.widget);
		  }

		} else {
          Error("parameter pada kode perintah Aksi_Tekan_Papanketik salah");
		}
        ...
        if(aksidat.datatype == 40) {
		  ...
		  if(aksidat.isGUI == 1) {        // window widget
		    g_signal_connect_swapped(G_OBJECT(aksidat.widget), "key-press-event",
		    G_CALLBACK(Do_AksiFungsiPressKeyboard), aksidat.widget);
		  }
