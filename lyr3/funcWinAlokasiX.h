...
// ambil pos x dari widget

VAL_LABEL funcWinAlokasiX ()
{
	  ...
	  switch( lex.type )
	    ...
	    case TYPE_IDENT:
              ...
              if(valdat.datatype == 40) {                       // Widget
                if(n == 0) {
                  datx.val = valdat.widget->allocation.x;   // alloc x
	              datx.datatype = 0;   // int
                } 
			  } else {
                Error("bentuk perintah Alokasi_X salah");
			  }
