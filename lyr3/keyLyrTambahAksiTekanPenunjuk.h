...
void keyAksi_Tekan_Penunjuk ()
{
  ...
  n = 0;
    ...
    if(n != 2) {
      Error("parameter Aksi_Tekan_Penunjuk tidak lengkap");
    } else {
	  ...
	  if(type_func == 1) {
        ...
        if(aksidat.datatype == 40) {
            ...
            g_object_set_data(G_OBJECT(aksidat.widget), "lbl_aksi_tekan_penunjuk", dat);  // GTK3

		  if(aksidat.isGUI == 1) {        // window widget
		    g_signal_connect_swapped(G_OBJECT(aksidat.widget), "button-press-event",
				G_CALLBACK(Do_AksiBendaFungsiMouse), aksidat.widget);
		  }

		} else {
          Error("parameter pada kode perintah Aksi_Tekan_Penunjuk salah");
		}
        ...
        if(aksidat.datatype == 40) {
		  ...
		  if(aksidat.isGUI == 1) {        // window widget
		    g_signal_connect_swapped(G_OBJECT(aksidat.widget), "button-press-event",
		    G_CALLBACK(Do_AksiFungsiPressMouse), aksidat.widget);
		  }
