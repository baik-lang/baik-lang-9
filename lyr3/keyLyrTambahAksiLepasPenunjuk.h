  ...
  n = 0;
	...
	do{
	  ...
	  switch( lex.type )
	    ...
	    default:
	      Error("bentuk perintah Aksi_Lepas_Penunjuk salah");
	    }
    ...
    if(n != 2) {
      Error("parameter Aksi_Lepas_Penunjuk tidak lengkap");
    } else {
	  // printf("Aksi_Tekan_Papanketik type1 %d\n", tmpdat.GUItype);
	  // printf("Jenis Fungsi %d\n", type_func);

      // add Aksi_Tekan_Penunjuk
      gtk_widget_add_events(aksidat.widget, GDK_BUTTON_RELEASE_MASK);
	  ...
	  if(type_func == 1) {
        ...
        if(aksidat.datatype == 40) {
          ...
          g_object_set_data(G_OBJECT( aksidat.widget), "lbl_aksi_lepas_penunjuk", dat);  // GTK3

		  if(aksidat.isGUI == 1) {        // window widget
		    g_signal_connect_swapped(G_OBJECT(aksidat.widget), "button-release-event",
		    G_CALLBACK(Do_AksiBendaFungsiMouse), aksidat.widget);
		  }

		} else {
          Error("parameter pada kode perintah Aksi_Lepas_Penunjuk salah");
		}
        ...
        if(aksidat.datatype == 40) {
		  ...
		  if(aksidat.isGUI == 1) {        // window widget
		    g_signal_connect_swapped(G_OBJECT(aksidat.widget), "button-release-event",
		    G_CALLBACK(Do_AksiFungsiReleaseMouse), aksidat.widget);
		  }
