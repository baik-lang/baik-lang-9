...
void keyPotongTeksPapanklip ()
{
  ...
  n = 0;
	    ...
	    case TYPE_IDENT:
             ...
             if(currentClass != NULL && strlen(currentClass) > 0) {
              ...
              if(valdat.datatype == 50) { // GtkTeksBuffer
                ...
                Error("bentuk perintah Potong_Teks_Papanklip salah");
			  }
    ...
    if(n != 2) {
      Error("parameter Potong_Teks_Papanklip tidak lengkap");
    } else {
	  if(tmpdat.datatype == 50) { // GtkTeksBuffer
		 gtk_text_buffer_cut_clipboard(tmpdat.buffer, tmpdat2.clipboard, 1); // cut ke clipboard
	  } else {
	     Error("parameter Potong_Teks_Papanklip harus TEKS_TERPILIH");
	  }
    }
