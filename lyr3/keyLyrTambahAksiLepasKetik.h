  ...
  n = 0;
	...
	do{
	  ...
	  switch( lex.type )
	    ...
	    case TYPE_IDENT:
             ...
             if(currentClass != NULL && strlen(currentClass) > 0) {
              ...
              if(aksidat.datatype == 40) {
                ...
                Error("bentuk perintah Aksi_Tekan_Papanketik salah");
			  }
	    ...
	    default:
	      Error("bentuk perintah Aksi_Lepas_Papanketik salah");
	    }
    ...
    if(n != 2) {
      Error("parameter Aksi_Lepas_Papanketik tidak lengkap");
    } else {
	  // printf("Aksi_Lepas_Papanketik type1 %d\n", tmpdat.GUItype);
	  // printf("Jenis Fungsi %d\n", type_func);

      // add Aksi_Lepas_Papanketik
      gtk_widget_add_events(aksidat.widget, GDK_KEY_RELEASE_MASK);
	  ...
	  if(type_func == 1) {
        ...
        if(aksidat.datatype == 40) {
          ...
          g_object_set_data(G_OBJECT(aksidat.widget), "lbl_aksi_releasekeyboard", dat);  // GTK3

		  if(aksidat.isGUI == 1) {        // window widget
		    g_signal_connect_swapped(G_OBJECT(aksidat.widget), "key-release-event",
		    G_CALLBACK(Do_AksiBendaFungsiReleaseKeyboard), aksidat.widget);
		  }

		} else {
          Error("parameter pada kode perintah Aksi_Lepas_Papanketik salah");
		}
        ...
        if(aksidat.datatype == 40) {
		  ...
		  if(aksidat.isGUI == 1) {        // window widget
		    g_signal_connect_swapped(G_OBJECT(aksidat.widget), "key-release-event",
		    G_CALLBACK(Do_AksiFungsiReleaseKeyboard), aksidat.widget);
		  }
