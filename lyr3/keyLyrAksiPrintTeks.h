...
void keyAksi_PrintTeks ()
{
  ...
  tmp_deep = sub_deep;
	  ...
	  switch( lex.type )
	    ...
	    case TYPE_IDENT:
              ...
              if(aksidat.datatype == 90) { // Print Ope
                ...
                Error("bentuk perintah aksi_cetak_teks salah");
			  }
    ...
    if(n != 2) {
      Error("parameter aksi_cetak_teks tidak lengkap");
    } else {
	  // printf("Aksi_Cetak_Teks tmpdat type %d GUItype %d\n", tmpdat.datatype, tmpdat.GUItype);
      ...
      if(type_func == 1) {
        ...
        if(tmpdat2.datatype == 90) {
            // printf("* call Do_AksiPrintTeks.......\n");

            if(tmpdat2.gtk_print_ope == NULL)
			  Error("salah operasi_cetak NULL");

			// printf("Do_AksiPrintTeks, aksi sub_name: %s\n", dat->sub_name);
			// printf("Do_AksiPrintTeks, aksi param_source: %s\n", dat->param_source);
			// printf("Do_AksiPrintTeks, aksi currentClass: %s\n", dat->currentClass);
 	        // printf("Do_AksiPrintTeks, aksi posSub: %d\n", dat->posSub);
	        // printf("Do_AksiPrintTeks, aksi posSubBack: %d\n", dat->posSubBack);

		  // call draw_page
          g_signal_connect (tmpdat2.gtk_print_ope, "draw_page", G_CALLBACK (Do_AksiPrintTeks), (gpointer)dat );

		} else {
          Error("parameter pada kode perintah aksi_cetak_teks salah 1");
		}


     } else if(type_func == 0) {
	    // // Do General Function
	    // printf("aksi_cetak_teks : general, datatype %d\n", tmpdat2.datatype);
		...
		// printf("Do_AksiPrintTeks, aksi currentClass: %s\n", dat->currentClass);
	        ...
	        // printf("Do_AksiPrintTeks, aksi posSubBack: %d\n", dat->posSubBack);
                 ...
                 //g_object_set_data(G_OBJECT( tmpdat2.widget), "lbl_aksi_print_teks", dat);  // GTK3

                 g_signal_connect (tmpdat2.gtk_print_ope, "draw_page", G_CALLBACK (Do_AksiPrintTeks), (gpointer)dat );


	} else {
          Error("parameter pada kode perintah aksi_cetak_teks salah 0");
	}
