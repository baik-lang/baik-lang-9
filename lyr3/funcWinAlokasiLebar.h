...
// ambil lebar dari widget

VAL_LABEL funcWinAlokasiLebar ()
{
              ...
              if(valdat.datatype == 40) {                       // Widget
                if(n == 0) {
				  if(valdat.widget != NULL) {
                    datx.val = valdat.widget->allocation.width;   // width
	                datx.datatype = 0;   // int
				  } else {
					Error("parameter nil : Alokasi_Lebar salah");
				  }
                } 
			  } else {
                Error("bentuk perintah Alokasi_Lebar salah");
			  }
