  ...
  // printf("keyTabBuku\n");
	do{
	  ...
	  switch( lex.type )
	    ...
	    default:
	      Error("bentuk perintah Tab_Buku salah");
	    }
    ...
    if(n != 2) {
      Error("parameter Tab_Buku tidak lengkap");
    } else {
	  if( ! strcmp(tmpMsg, "ATAS") || ! strcmp(tmpMsg, "atas")) {
        gtk_notebook_set_tab_pos (GTK_NOTEBOOK (tmpdat.widget), GTK_POS_TOP);
	  } else if ( ! strcmp(tmpMsg, "BAWAH") || ! strcmp(tmpMsg, "bawah")) {
        gtk_notebook_set_tab_pos (GTK_NOTEBOOK (tmpdat.widget), GTK_POS_BOTTOM);
	  } else {
        Error("parameter Tab_Buku tidak benar");
	  }
    }
