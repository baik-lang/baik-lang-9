...
void keyAksi_Atur ()
{
  ...
  n = 0;
	    ...
	    case TYPE_IDENT:
             ...
             if(currentClass != NULL && strlen(currentClass) > 0) {
              ...
              if(aksidat.datatype == 40) {
                ...
                Error("bentuk perintah Aksi_Atur salah");
			  }
    ...
    if(n != 2) {
      Error("parameter Aksi_Atur tidak lengkap");
    } else {
	  // printf("Aksi_Atur type1 %d\n", tmpdat.GUItype);
	  // printf("Jenis Fungsi %d\n", type_func);

      // add MASK of CONFIGURE EVENT
      gtk_widget_add_events(aksidat.widget, GDK_STRUCTURE_MASK);
	  ...
	  if(type_func == 1) {
        ...
        if(aksidat.datatype == 40) {
		  if(aksidat.isGUI == 1) {        // window widget
		    //g_signal_connect(G_OBJECT(aksidat.widget), "configure_event",
		    //GTK_SIGNAL_FUNC(Do_AksiBendaFungsiAtur), dat);
		  }

		} else {
          Error("parameter pada kode perintah Aksi_Atur salah");
		}
        ...
        if(aksidat.datatype == 40) {
		  g_object_set_data(G_OBJECT(aksidat.widget), "cairo_aksi_atur", dat);  // GTK3

		  if(aksidat.isGUI == 1) {        // window widget
			  g_signal_connect_swapped(G_OBJECT(aksidat.widget), "configure_event",
				G_CALLBACK(Do_AksiFungsiAtur), aksidat.widget);
		  }
