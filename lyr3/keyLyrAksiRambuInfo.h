...
void keyAksi_Rambu_Info()
{
	...
	do{
		...
		default:
			Error("bentuk perintah RambuInfo salah");
		}
	...
	//printf("aksi rambu INFO title=%s \n", title);
	//printf("msg=%s \n", msg);
	//printf("logo=%s \n", logo);
	//printf("n=%d --------- \n", n);
	...
	if (n == 3) {
		...
		g_object_set_data(G_OBJECT(tmpdat.widget), "lbl_aksi_info", dat);  // GTK3

		if (tmpdat.GUItype == 3) { // Button, NOT window widget
			g_signal_connect_swapped(G_OBJECT(tmpdat.widget), "clicked",
				G_CALLBACK(win_rambu_info), tmpdat.widget);
		}
		...
		Error("Aksi_Rambu_Info tidak lengkap");
	}
