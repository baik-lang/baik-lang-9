...
// ambil Tinggi dari widget

VAL_LABEL funcWinAlokasiTinggi ()
{
	  ...
	  switch( lex.type )
	    ...
	    case TYPE_IDENT:
              ...
              if(valdat.datatype == 40) {                       // Widget
                if(n == 0) {
			  datx.val = valdat.widget->allocation.height;   // height
	              datx.datatype = 0;   // int
                } 
			  } else {
                Error("bentuk perintah Alokasi_Tinggi salah");
			  }
