  ...
  tmp_deep = sub_deep;
        ...
        if(aksidat.datatype == 40) {
		  if(aksidat.GUItype == 0) {        // window widget
		    ...
		    g_signal_connect(G_OBJECT(aksidat.widget), "activate",
				G_CALLBACK(Do_AksiBendaFungsi), dat);
	...
	// printf("aksi_fungsi, deep %d pos %d \n", lyr_sub_deep, posLyrSub);
        ...
        if(aksidat.datatype == 40) {
          ...
          g_object_set_data(G_OBJECT(tmpdat.widget), "lbl_aksi_fungsi", dat);  // GTK3
		  ...
		  if(aksidat.GUItype == 0) {        // window widget
		    ...
		    g_signal_connect_swapped(G_OBJECT(aksidat.widget), "activate",
				G_CALLBACK(Do_AksiFungsi), aksidat.widget);
		    ...
		    g_signal_connect_swapped(G_OBJECT(aksidat.widget), "clicked",
				G_CALLBACK(Do_AksiFungsi), aksidat.widget);
		    ...
		    g_signal_connect_swapped(G_OBJECT(aksidat.widget), "button-press-event",
				G_CALLBACK(Do_AksiFungsiTreeMouse), aksidat.widget);
		  }
