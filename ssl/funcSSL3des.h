...
VAL_LABEL funcSSL_3des_encrypt ()
{
  VAL_LABEL datx;
  VAL_LABEL valdat;
  int n=0;
  ...
  do {
    ...
    if(lex.type == TYPE_IDENT) {
           ...
           if(datx.datatype == 3) {
             ...
             Error("enkripsi_3des: masukan data salah");
           }
  ...
  if(n == 2) {
    ...
    memset(&datx, '\0', sizeof(datx));
	encrypt_3des(plaintext, (char *) &ciphertext, mykey);
...
VAL_LABEL funcSSL_3des_decrypt ()
{
  VAL_LABEL datx;
  VAL_LABEL valdat;
  int n=0;
  ...
  do {
    ...
    if(lex.type == TYPE_IDENT) {
		   ...
		   if(datx.datatype == 3) {
             ...
             Error("dekripsi_3des: masukan data salah");
           }
	} else if(lex.type == TYPE_STR) {
      if(n == 0) {
  ...
  if(n == 2) {
    ...
    memset(&datx, '\0', sizeof(datx));
	decrypt_3des((char *) &plaintext, ciphertext, mykey);
