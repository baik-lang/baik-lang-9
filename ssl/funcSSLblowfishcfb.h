...
VAL_LABEL funcSSL_blowfishcfb_encrypt ()
{
  VAL_LABEL datx;
  VAL_LABEL valdat;
  int n=0;
  ...
  do {
    ...
    if(lex.type == TYPE_IDENT) {
           ...
           if(datx.datatype == 3) {
             ...
             Error("enkripsi_blowfishcfb: masukan data salah");
           }
  ...
  if(n == 2) {
    //printf ("enkripsi blowfishcfb %s\n", plaintext);

    memset(&datx, '\0', sizeof(datx));
	encrypt_blowfishcfb(plaintext, (char *)&ciphertext, mykey);
...
VAL_LABEL funcSSL_blowfishcfb_decrypt ()
{
  VAL_LABEL datx;
  VAL_LABEL valdat;
  int n=0;
  ...
  do {
    ...
    if(lex.type == TYPE_IDENT) {
		   ...
		   if(datx.datatype == 3) {
             ...
             Error("dekripsi_blowfishcfb: masukan data salah");
           }
	} else if(lex.type == TYPE_STR) {
      if(n == 0) {
  ...
  if(n == 2) {
    ...
    memset(&datx, '\0', sizeof(datx));
	decrypt_blowfishcfb((char *)&plaintext, ciphertext, mykey);
