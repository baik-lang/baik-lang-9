...
VAL_LABEL funcWinExpander ()
{
  ...
  memset(tmpMsg, '\0', sizeof(tmpMsg));
           ...
           strcpy(tmpMsg, lex.detail.string );
            ...
            if(strlen(lex.detail.array_str) > 0) {
              ...
              if(valdat.datatype == 8) {
                ...
                Error("Perinci: masukan data salah");
              }
           }
		 ...
		 // make new label
		 datx.widget   = gtk_expander_new(g_locale_to_utf8(tmpMsg, -1,NULL,NULL,NULL));
         // make also eventbox for parent/base canvas
		 datx.eventbox = gtk_event_box_new();

		 datx.isGUI    = 1;
		 datx.GUItype  = 11;    // Expander - perinci - see ValLabel
