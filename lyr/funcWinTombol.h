...
VAL_LABEL funcWinTombol ()
{
  ...
  memset(tmpMsg, '\0', sizeof(tmpMsg));
           ...
           sprintf(tmpMsg, "%li", lex.detail.num);
			...
			if(strarryPos < 0) {
			  ...
			  strcpy(tmpMsg, TmpStrBox.str.mystring);
                ...
                Error("TOMBOL: masukan data salah");
              }
           }
		 ...
		 //datx.widget   = gtk_button_new_with_label(tmpMsg);
		 datx.widget   = gtk_button_new_with_label(g_locale_to_utf8(tmpMsg, -1,NULL,NULL,NULL));

         // make also eventbox for parent/base canvas
		 // datx.eventbox = gtk_event_box_new();

		 datx.isGUI    = 1;
		 datx.GUItype  = 3;    // Button - tombol - see ValLabel
