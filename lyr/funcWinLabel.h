  ...
  memset(tmpMsg, '\0', sizeof(tmpMsg));
         ...
         if(lex.type == TYPE_STR) {
		  ...
		  strcpy(TmpStrBox.var.array_name, valdat.array_name);
                ...
                Error("LABEL: masukan data salah");
              }
           }
		...
		strcpy(tmpMsg, datx.str);
		 ...
		 // make new label
		 // datx.widget   = gtk_label_new(tmpMsg);
         datx.widget=gtk_label_new(g_locale_to_utf8(tmpMsg, -1,NULL,NULL,NULL));

         // make also eventbox for parent/base canvas
		 datx.eventbox = gtk_event_box_new();

		 datx.isGUI    = 1;
		 datx.GUItype  = 4;    // Label - label - see ValLabel
