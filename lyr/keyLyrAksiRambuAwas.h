  ...
  memset(&class_tmpvar, '\0', sizeof(class_tmpvar));
	...
	do{
	  ...
	  switch( lex.type )
	    ...
	    default:
	      Error("bentuk perintah AksiRambuAwas salah");
	    }
    ...
    //printf("aksi rambu AWAS title=%s \n", title);
    //printf("msg=%s \n", msg);
    //printf("logo=%s \n", logo);
    //printf("n=%d --------- \n", n);
    ...
    if(n == 3) {
      ...
      gtk_object_set_data(GTK_OBJECT(tmpdat.widget), "lbl_aksi_warn", dat); 
	  
	  if(tmpdat.GUItype == 3) { // Button, NOT window widget
          g_signal_connect_swapped(G_OBJECT(tmpdat.widget), "clicked",
				  G_CALLBACK(win_rambu_warning), tmpdat.widget);
      ...
      Error("Aksi_Rambu_Awas tidak lengkap");
	}
