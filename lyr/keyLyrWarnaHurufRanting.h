...
void keyWarnaHurufRanting ()
{
  ...
  n = 0;
	  ...
	  switch( lex.type )
		...
		valdat = ValLabel( lex.detail.ident, sub_deep, valdat, VAL_FLAG_SEARCH_R );
              ...
              if(valdat.datatype == 3) {
                ...
                Error("bentuk perintah warnahuruf_sel salah");
			  }
    ...
    if(n != 2) {
      Error("parameter warnahuruf_sel tidak lengkap");
    } else {
		...
		// printf("warna huruf SEL %d %s\n", tmpdat.datatype, warna);

        if(tmpdat.datatype == 57) {
          g_object_set(tmpdat.renderer,"foreground",warna, "foreground-set", TRUE, NULL);
		}
    }
